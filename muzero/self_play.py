...
Original file is located at
    https://colab.research.google.com/github/sugiyama404/ReinfoceLearningForTrading/blob/main/MuZero/self_play.ipynb
"""
...
class SharedStorage:
    ...
    def latest_network(self, laod_model = False) -> Network:
        if self._networks:
            return self._networks[max(self._networks.keys())]
        else:
            # policy -> uniform, value -> 0, reward -> 0
            return self._make_uniform_network(laod_model)

    def save_network(self, step: int, network: Network):
        self._networks[step] = network

    def _make_uniform_network(self, laod_model = False):
        return Network(laod_model)
...
def run_selfplay(config: MuZeroConfig, storage: SharedStorage, replay_buffer: ReplayBuffer):
    ...
    network = storage.latest_network(laod_model = True)
...
def muzero(config: MuZeroConfig):
    ...
    for _ in range(config.num_actors):
        '''config.num_actors: 4'''
        p = Process(target=launch_job, args=(run_selfplay, config, storage,
                                             replay_buffer, lock))
        worker.append(p)
        p.start()
    for w in worker:
...
def launch_job(f, config, storage, replay_buffer, lock):
    os.environ["CUDA_VISIBLE_DEVICES"] = ""
    file_name = game_dir + '/muzero_game.pkl'
    if os.path.isfile(file_name):
        replay_buffer = load(file_name)
        print(f'loading: {str(len(replay_buffer.buffer))} pieces')

    for i in range(2):
        start_time = datetime.now()
        f(config, storage, replay_buffer)
        lock.acquire()
        if os.path.isfile(file_name):
            tmp_replay_buffer = load(file_name)
            tmp_replay_buffer.save_game(replay_buffer.buffer[-1])
            replay_buffer = tmp_replay_buffer
        save(file_name, replay_buffer)
        lock.release()
        end_time = datetime.now() - start_time
        print(f'time: {end_time} game nums: {str(len(replay_buffer.buffer))} pieces')
...
#保存
def save(file_name, replay_buffer):
    with open(file_name, "wb") as f:
        pickle.dump(replay_buffer, f)

muzero(make_trade_config())