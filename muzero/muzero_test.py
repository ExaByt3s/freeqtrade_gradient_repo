...
Original file is located at
    https://colab.research.google.com/github/sugiyama404/ReinfoceLearningForTrading/blob/main/MuZero/MuZero_test.ipynb
"""
...
def run_selfplay(config: MuZeroConfig, storage: SharedStorage, actor_num: int=99):
    ...
    with open(csv_path, 'a') as f:
        row = str(game.info['cur_revenue']) + ',' + str(game.info['trade_time']) + ',' + str(game.info['trade_win'])
        print(row, file=f)
    return game
...
def muzero(config: MuZeroConfig):
    ...
    for i in range(config.num_actors):
        ...
        p = Process(target=launch_job,
                    args=(run_selfplay, config, storage, i))
        worker.append(p)
        p.start()
    for w in worker:
        w.join()

def launch_job(f, config: MuZeroConfig, storage:SharedStorage, actor_num: int):
    os.environ["CUDA_VISIBLE_DEVICES"] = ""
    for i in range(25):
        start_time = datetime.now()
        _ = f(config, storage, 0)
        end_time = datetime.now() - start_time
        print(f'actor_num: No.{str(actor_num + 1)} time: {end_time}')
